...
def main():
    ...
    console.print(Panel.fit(
        "[bold cyan]🔐 Быстрое исправление SSL через Let's Encrypt[/bold cyan]",
        border_style="cyan"
    ))
    
    console.print("\n[yellow]Этот скрипт:[/yellow]")
    console.print("1. Переключит Caddy на Let's Encrypt (работает без проблем с DNS)")
    ...
    console.print("\n[green]✓ Let's Encrypt - стандартный CA для Caddy[/green]")
    ...
    # 1. Переключаем на Let's Encrypt
    console.print("\n[bold cyan]Шаг 1: Переключение на Let's Encrypt[/bold cyan]")
    try:
        from switch_to_letsencrypt import switch_to_letsencrypt, clear_old_certificates, regenerate_caddyfile
        
        if switch_to_letsencrypt():
            console.print("[green]✓ Переключено на Let's Encrypt[/green]")
        else:
            console.print("[yellow]⚠ Let's Encrypt уже настроен или ошибка[/yellow]")
    except ImportError:
        console.print("[red]❌ Не удалось импортировать switch_to_letsencrypt.py[/red]")
