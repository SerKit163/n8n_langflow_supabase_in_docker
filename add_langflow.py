...
def configure_langflow(hardware, existing_config):
    ...
    if routing_mode == 'subdomain':
        ...
        use_auto = Confirm.ask(
            "Автоматически сформировать поддомен для Langflow?",
            default=True
        )
        
        if use_auto and base_domain:
            # АВТОМАТИЧЕСКИЙ РЕЖИМ
            auto_domain = f"langflow.{base_domain}"
            console.print(f"\n[green]✓ Предложенный домен: {auto_domain}[/green]")
            if Confirm.ask(f"Использовать домен {auto_domain}?", default=True):
                langflow_config['langflow_domain'] = auto_domain
            else:
                # Ручной ввод
                while True:
                    langflow_domain = Prompt.ask(
                        "Домен для Langflow (например, langflow.example.com) или '-' для пропуска",
                        default=existing_config.get('LANGFLOW_DOMAIN', auto_domain)
                    )
                    if langflow_domain == '-':
        ...
        if base_domain:
            # Предлагаем автоматический или ручной режим
            use_auto = Confirm.ask(
                "Автоматически использовать путь /langflow?",
                default=True
            )
            ...
            if use_auto:
                ...
                if Confirm.ask(f"Использовать путь {auto_path}?", default=True):
                    langflow_config['langflow_path'] = auto_path
                    langflow_config['base_domain'] = base_domain
                else:
                    # Ручной ввод
                    while True:
                        langflow_path = Prompt.ask(
                            "Путь для Langflow (например, /langflow)",
                            default=existing_config.get('LANGFLOW_PATH', '/langflow')
                        )
                        is_valid, error = validate_path(langflow_path)
                        if is_valid:
            ...
            while True:
                ...
                if is_valid:
                    langflow_config['base_domain'] = base_domain
                    # Предлагаем путь
                    use_auto = Confirm.ask("Автоматически использовать путь /langflow?", default=True)
                    if use_auto:
                        langflow_config['langflow_path'] = '/langflow'
                    else:
                        while True:
                            langflow_path = Prompt.ask("Путь для Langflow", default="/langflow")
                            is_valid, error = validate_path(langflow_path)
                            if is_valid:
                                langflow_config['langflow_path'] = langflow_path
                                break
...
def start_langflow():
    ...
    if Confirm.ask("Запустить Langflow сейчас?", default=True):
        ...
        if docker_compose_up(detach=True):
            ...
            console.print("[red]❌ Ошибка при запуске Langflow[/red]")
